...
public class TestDataTags extends BaseDataAccessTest {
	...
	public void testTagETL2(){
		...
		BulkFactories.getBulkTagParticipationFactory().setBatchSize(1000);
		
		//Factories.getTagParticipationFactory().setUseThreadSafeCollections(false);
		Factories.getTagParticipationFactory().setAggressiveKeyFlush(false);
		Factories.getGroupFactory().setUseThreadSafeCollections(false);
		Factories.getDataFactory().setUseThreadSafeCollections(false);
		Factories.getTagFactory().setUseThreadSafeCollections(false);
	...
	public void testTagETL(){
		...
		try{
			...
			for(int i = 0; i < dataFile.length;i++){
				...
				String name = limitNames.matcher(pairs[1]).replaceAll("0");
				DataType data = Factories.getDataFactory().getDataByName(name, true, dir);
				if(data == null){
					logger.warn("Failed to find data '" + name + "' in '" + path +"'");
					continue;
				}
				DataParticipantType dpt = Factories.getTagParticipationFactory().newDataTagParticipation(nTag, data);
				BulkFactories.getBulkFactory().createBulkEntry(sessionId, FactoryEnumType.TAGPARTICIPATION, dpt);
	...
	public void testDataTags(){
		...
		try {
			...
			parts = Factories.getTagParticipationFactory().convertList(Factories.getTagParticipationFactory().getTagParticipations(new DataTagType[]{tag1,tag2}, null,ParticipantEnumType.DATA));
			logger.info("Parts = " + parts.size());
