# org, username, targetUserName, newUserPassword
...
if [ -f $userCacheName ]; then
   ...
   echo "Composing add $1/$3 organization request in $1 (id: $userObjectId)..."
   # create the new credential stub
   res=$(./genPwdAuth.sh "$1" "$3" "$4")

   authName=$(./hash.sh "$1.$2")
   newAuthName=$(./hash.sh "$1.$3")
   url=$(./encode.sh "http://127.0.0.1:8080/AccountManagerService/rest/token/credential/USER/$userObjectId")
